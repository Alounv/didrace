...
export const getPlayerTypedWords = query({
  ...
  handler: async (ctx, args) => {
    ...
    const playerId = args.playerId ?? userID;
    
    if (!playerId) {
      return [];
    }
    
    let query = ctx.db.query("typedWords").withIndex("by_player", (q) => q.eq("playerID", playerId));
    
    if (args.raceId) {
      query = ctx.db.query("typedWords").withIndex("by_player_race", (q) => 
...
export const getRaceAnalytics = query({
  ...
  handler: async (ctx, args) => {
    ...
    // Group by player
    const playerStats = typedWords.reduce((acc, word) => {
